...
| `chown`, `fchownat` | emulated | P; server permission/squash checks; explicit numeric/domain identity mapping, unchanged-ID sentinel handling. **Implementation note (M1):** `fchownat` decodes the sentinel and honours it by refusing the shapes that cannot preserve it. Since [#60](https://github.com/invakid404/umbra/issues/60) that refusal is per object rather than blanket: copy-up carries the base object's uid/gid onto the shadow where the backend advertises `ownership-fidelity-v1` and privilege permits, and the unchanged-ID refusal stands only where the overlay cannot guarantee the carry. Identity mapping is still not built, so raw uid/gid reach the kernel against the shadow object. |
